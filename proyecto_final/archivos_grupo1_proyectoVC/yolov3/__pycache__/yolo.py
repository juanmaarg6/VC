from tensorflow.keras.layers import Conv2D, Input, BatchNormalization, LeakyReLU, ZeroPadding2D, UpSampling2D, Lambda, add, concatenate
...
class YoloLayer(Layer):
    ...
    def call(self, x):
        ...
        if debug:
		tf.print("avg_obj \t\t", avg_obj, summarize=1000)
		tf.print("avg_noobj \t\t", avg_noobj, summarize=1000)
	    	tf.print("avg_iou \t\t", avg_iou, summarize=1000)
	    	tf.print("avg_cat \t\t", avg_cat, summarize=1000)
	    	tf.print("recall50 \t", recall50, summarize=1000)
	    	tf.print("recall75 \t", recall75, summarize=1000)
	    	tf.print("count \t", count, summarize=1000)
	    	tf.print("loss xy, wh, conf, class: \t", 
